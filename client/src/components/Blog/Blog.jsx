...
export default function Blog() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8000/api/user/post/${id}`)
      .then((res) => {
  ...
  const handleLike = (id) => {
    if (loggedIn) {
      if (!liked) {
        axios
          .post(`http://localhost:8000/api/user/post/like/${id}`, {
            user_id: userId,
          })
          .then((res) => {
            setBlog(res.data.blog);
        ...
        axios
          .post(
            `http://localhost:8000/api/user/post/unlike/${id}`,
            {
              user_id: userId,
            }
          )
  ...
  const addComment = (id) => {
    axios
      .post(`http://localhost:8000/api/user/post/comment/${id}`, {
        user_id: userId,
